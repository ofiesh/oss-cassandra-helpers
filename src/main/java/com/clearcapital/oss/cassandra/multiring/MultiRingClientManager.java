...
public class MultiRingClientManager {
    ...
    private static Logger log = LoggerFactory.getLogger(MultiRingClientManager.class);
    ...
    /**
     * Find the ring for the specified groupKey. Note that if the specified groupKey is not configured, this will grab
     * the configured default ring.
     */
    ...
    private static ImmutableMap<String, RingClient> buildConnections(MultiRingClientMapper mapper)
            throws AssertException {
        ImmutableMap.Builder<String, RingClient> connectionsBuilder = ImmutableMap.<String, RingClient> builder();
    ...
    public boolean keyspacesExist() throws AssertException {
        boolean result = true;
        ImmutableMap<String, RingClient> ringClients = getRingClients();
        for( Entry<String, RingClient> e :ringClients.entrySet()) {
            RingClient client = e.getValue();
            String preferredKeyspace = client.getPreferredKeyspaceName();
            if (!client.keyspaceExists(preferredKeyspace)) {
                result = false;
                log.warn(e.getKey() + ": keyspace \"" + preferredKeyspace + "\" does not exist.");
            }
        }
        return result;
    }
